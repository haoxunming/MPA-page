...
const user_info = JSON.parse(window.client.getGameUserInfo())
// eslint-disable-next-line camelcase
const jwt_token = JSON.parse(window.client.getHeaderParams())['jwt-token']

const userInfo = ref({
  userId: user_info.mUserId,
  nickName: user_info.mNickName,
  inviteId: user_info.mInviteId,
  country: user_info.mCountry,
  token: jwt_token,
  platform: '6'
})
...
const mailDetails = ref<any>(null)
